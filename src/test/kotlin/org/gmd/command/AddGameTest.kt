...
class AddGameTest {
    ...
    fun addGameShouldPrintFeedbackOfTheAddedGame() {
        val helper = SlackResponseHelper()
        ...
        AddGame(response = helper, envProvider = EnvProviderForTesting(emptyMap(), timestamp), service = gameService, account = account, tournament = tournament).parse(listOf(player1, player2))
    ...
    fun addGameShouldPrintEloEvolutionIfRequired() {
        ...
        AddGame(response = helper, envProvider = EnvProviderForTesting(emptyMap(), timestamp), service = gameService, account = account, tournament = tournament).parse(listOf("--with-elo", player1, player2))

        Assert.assertEquals("Good game! A new game entry has been created!", helper.slackResponse.text)
        Assert.assertEquals("1. player1\n2. player2", helper.slackResponse.attachments.first().text)
        Assert.assertEquals("1. player1 (1213, +13)\n2. player2 (1193, -7)", helper.slackResponse.attachments.last().text)
        Assert.assertEquals("in_channel", helper.slackResponse.responseType)
    }
