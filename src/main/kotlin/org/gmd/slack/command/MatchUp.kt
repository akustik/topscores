...
class MatchUp(
    ...
    private val player1 by argument(help = "Player1 name")
    private val player2 by argument(help = "Player2 name").default(username)
    ...
    private fun returnEvolution() {
        ...
        val winRatio = calculateWinRatioForA(games, team1, team2)

        if (winRatio == null) {
            response.asyncMessage(text = "No matches found", silent = silent)
        } else {
            response.asyncMessage(text = "$player1 beats $player2 ${Math.round(winRatio.second * 100).toInt()}% of the times (sample: ${winRatio.first} games)", silent = silent)
        }
    }
